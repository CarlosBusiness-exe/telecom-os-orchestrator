...
# --- LÓGICA V1: Funções Originais Restauradas ---
...
async def fetch_order_by_filter(cidade: str, tipo: str):
    """Lógica de filtro original da sua V1"""
...
async def create_marker_list(cidade: str = "catalao", tipo: str = "suporte"):
    # Dicionários de mapeamento originais
    ...
    for order in order_list:
        try:
            ...
            # Formatação exata baseada na sua imagem da V1
            # Note que a V1 incluía campos fixos como LOGIN, SENHA, PAINEL
            clean_desc = order['order_desc'].replace('\r\n', '<br>').replace('\n', '<br>')
            
            # Montagem do conteúdo do balão igual à Image 1
            balloon_content = (
                ...
                f"SENHA: None<br>"
                f"PAINEL: None<br><br>"
                f"Localização: {order['anotacao_tecnica']}<br>"
                f"Endereço: {order['endereco']}, {order['bairro']}, {order['cidade']}<br><br>"
                f"Solucionar:<br>"
                f"{clean_desc}<br>"
                f"===========================================<br>"
                f"===="
